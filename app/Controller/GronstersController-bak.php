 ...
 */
	...
	* @var array
	*/
	public $components = array('GronsterImage', 'Session');
	...
	public function view($id=null){
		...
		switch($total) {
			case 1:
				...
				$start_message = "Draw the middle part.";
			...
			case 2:
				...
				*  show them the bottom part of the middle panel so they can connect to it. */
				...
				$start_message = "Draw the bottom part.";	
				$helpmessage = "A Gronster is a drawing made of three sections: a head, a body, and legs. 
	...
	public function add() {
	    ...
	    if ($this->request->is('post')) {
		    if (!empty($this->request->data)) {
  			...
  			if ($this->Session->read('visited') == false) {
  				$this->Session->write('visited',true);
  				$this->redirect(array('action' => 'welcome'));
			} else {
				$this -> set('start_message', "Start by drawing the top.");
				$this->set('helptitle',"A Little Help");
				$helpmessage = "A Gronster is a drawing made of three sections: a head, a body, and legs. 
  ...
  public function getPanelCount($id){
	...
	public function welcome(){
		...
		$this->Session->write('visited',true); // session var 'visited' set to true	
	}
	...
	*/
	public function checkControllerClicks(){
		$clicks = 1 + $this->Session->read('controllerClicks');
