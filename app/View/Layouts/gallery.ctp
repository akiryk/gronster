...
<head>
	...
	<?php
		// JAVASCRIPT FILES
		// Other scripts are loaded in the controllers (view, add, etc) using jsIncludes variable -- see writebuffer below
			...
			<ul id="navigation">
				<li>
					<?php 
						...
						echo $this->Html->link('<span>Start</span>',
				...
				<li>
					<?php
						if (	$this->Html->action == "add" 
							...
							|| 	$this->Html->action == "viewFinished") 
						{
							$class="active";
						} else { 
							$class = ""; 
						} 
						echo $this->Html->link('<span>Create a gronster</span>',
